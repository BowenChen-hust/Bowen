...
os.environ["CUDA_VISIBLE_DEVICES"]= "0" if not 'home' in sys.path[0] else "0"
...
def activate():
  ...
  model = th.model()
  model.mark ='lr{}-bs{}-f{}-t{}-b{}-l{}-{}{}{}-{}{}{}-{}{}{}'.format(th.learning_rate,
    th.batch_size,  th.feature_type, th.target_type, th.boost, model.loss.name,
  ...
  agent = Agent(model.mark, th.task_name)
  agent.config_dir(__file__)

  print(agent.data_dir)
  if  'yiz/share/projects/lambai_v2/01-PR/data' in agent.data_dir:
    agent.data_dir = agent.data_dir.replace('yiz/share/projects/lambai_v2/01-PR', 'D:/learning/lambai_v2-main/01-PR/data')
